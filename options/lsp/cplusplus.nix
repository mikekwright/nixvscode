{ pkgs, extra-pkgs, funcs, system, options, ... }:

{
  vscodeExtensions = with extra-pkgs.extensions; options.extensions "lsp.cplusplus" [
  ...
  packages = with pkgs; options.packages "lsp.cplusplus" [
    clang-tools
    cmake
  ];
