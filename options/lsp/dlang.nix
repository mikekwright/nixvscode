{ pkgs, extra-pkgs, system, funcs, options, ... }:
...
in {
  vscodeExtensions = with extra-pkgs.extensions; options.extensions "lsp.dlang" [
    # Dlang main extension
    #    https://marketplace.visualstudio.com/items?itemName=webfreak.code-d
    (funcs.safePkg vscode-marketplace [ "webfreak" "code-d" ])
    ...
    (funcs.safePkg vscode-marketplace [ "hbenl" "vscode-test-explorer" ])
    ...
    (funcs.safePkg vscode-marketplace [ "ms-vscode" "test-adapter-converter" ])
  ];

  packages = with pkgs; options.packages "lsp.dlang" [
    #dmd
    #gdc
